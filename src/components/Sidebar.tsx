...
const studentAffairsLinks: NavItem[] = [
  {
    label: "Dashboard",
    href: "/dashboard",
    icon: (
...
function getNavLinks(role: Role, basePath: string): NavItem[] {
  let links: NavItem[];
  switch (role) {
    case "admin":
      links = adminLinks;
  ...
  return links.map((link) => ({ ...link, href: `${basePath}${link.href}` }));
}
...
export default function Sidebar({ role, userName, isOpen, onClose, basePath }: SidebarProps) {
  const pathname = usePathname();
  const links = getNavLinks(role, basePath);

  return (
    <>
      <div
        className={`fixed inset-0 bg-black/50 z-40 transition-opacity duration-200 ease-out lg:hidden ${
      ...
      >
        ...
        <div className="border-t border-border p-4">
          ...
          <button
            ...
            className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-secondary-600 hover:bg-danger-light hover:text-danger transition-colors"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
